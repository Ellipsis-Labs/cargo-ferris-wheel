...
Place unit tests beside implementations via `mod tests`. Integration coverage lives in `tests/`, named `<feature>_test.rs`. Use `assert_cmd`, `predicates`, and `tempfile` for CLI scenarios, and match on concrete error variants. Every feature should pass `cargo nextest run --profile ci`; refresh any fixtures or snapshots you modify.
