...
impl GraphRenderer {
    ...
    pub fn render_ascii(
        ...
        for node_idx in nodes {
            ...
            let in_cycle = cycles_ws_names.iter().any(|cycle| cycle.contains(ws_name));
                    ...
                    if self.show_crates {
                        ...
                        for (j, edge) in edges.iter().enumerate() {
                            ...
                            writeln_out!(
                                ...
                                edge.from_crate,
                                edge.to_crate,
                                edge.target.as_deref().unwrap_or("all targets")
                            )?;
                        }
                    }
                }
    ...
    pub fn render_mermaid(
        ...
        for (prefix, nodes) in groups.iter() {
            ...
            for &node in nodes {
                let ws = &graph[node];
                let in_cycle = cycles_ws_names.iter().any(|cycle| cycle.contains(&ws.name));
    ...
    pub fn render_cycle_summary(
        ...
        for (i, cycle) in cycles.iter().enumerate() {
            ...
            for edge in cycle.edges() {
                *type_counts
                    .entry(edge.dependency_type.as_str())
                    .or_insert(0) += 1;
            }
    ...
    fn calculate_cycle_severity(&self, cycle: &WorkspaceCycle) -> CycleSeverity {
        ...
        for edge in edges {
            match edge.dependency_type.as_str() {
