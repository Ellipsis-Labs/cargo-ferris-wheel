...
use crate::analyzer::{Dependency, WorkspaceInfo};
...
impl DependencyGraphBuilder {
        ...
        for ws_info in workspaces.values() {
            ...
            for member in &ws_info.members {
                let node = WorkspaceNode::builder()
                    ...
                    .with_crates(vec![member.name.clone()])
                    .build()
                    .wrap_err("Failed to build WorkspaceNode")?;

                let idx = self.graph.add_node(node);
                crate_indices.insert(member.name.clone(), idx);
            }
        }
    ...
    pub fn build_cross_workspace_graph(
        ...
        for (ws_path, ws_info) in workspaces {
            let node = WorkspaceNode::builder()
                .with_name(ws_info.name.clone())
                .with_crates(ws_info.members.iter().map(|m| m.name.clone()).collect())
                .build()
                .wrap_err("Failed to build WorkspaceNode")?;
    ...
    fn load_workspace_dependencies(
        ...
        for ws_path in workspaces.keys() {
            ...
            if workspace_toml_path.exists() {
                if let Ok(cargo_toml) = CargoToml::parse_file(&workspace_toml_path) {
    ...
    fn process_dependency(
        ...
        if let Some(target_ws_path) = crate_to_workspace.get(&dep.name) {
            ...
            {
                if let Some(dep_path) = workspace_deps.get(&dep.name) {
                    ...
                    for (crate_name, ws_path) in crate_to_workspace {
                        ...
                        {
                            if let Some(&to_ws_idx) = self.workspace_indices.get(ws_path) {
                                if from_ws_idx != to_ws_idx {
                                    let edge = DependencyEdge::builder()
                                        .with_from_crate(from_crate)
    ...
    fn test_build_simple_graph() {
        ...
        workspaces.insert(
            ...
            WorkspaceInfo {
                ...
                members: vec![test_crate_member("crate-b", &ws_b_path, vec![])],
                is_standalone: false,
            },
        );
            ...
            WorkspaceInfo {
                ...
                members: vec![
                    ...
                    CrateMember::builder()
                        ...
                        .with_dev_dependencies(vec![Dependency {
            ...
            WorkspaceInfo {
                ...
                members: vec![
                    ...
                    CrateMember::builder()
                        ...
                        .with_build_dependencies(vec![Dependency {
                            name: "crate-b".to_string(),
                            target: None,
                        }])
                        .build()
                ...
                members: vec![
                    CrateMember::builder()
                        ...
                        .with_dependencies(vec![
                            Dependency {
                                name: "crate-b".to_string(), // Internal dependency
                                target: None,
                            },
                            Dependency {
                                name: "external-crate".to_string(), /* External dependency
                                                                     * (should be ignored) */
                                target: None,
                            },
                        ])
