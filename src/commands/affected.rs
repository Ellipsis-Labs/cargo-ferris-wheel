...
impl AffectedAnalysis {
    ...
    pub fn analyze_affected_files(&self, files: &[String]) -> AffectedResult {
        ...
        for file in files {
            ...
            // Track if we handled this as a workspace-level Cargo file
            let mut handled_as_workspace_cargo = false;

            // First, check if this is a workspace-level Cargo file
            if is_cargo_file {
                ...
                for ws_path in self.workspaces.keys() {
                        ...
                        && parent == abs_ws_path
                    {
                        ...
                        for ((crate_name, crate_path), crate_ws_path) in
                        ...
                        handled_as_workspace_cargo = true;
                        break;
                    }
                }
            }

            // If we handled this as a workspace-level Cargo file, skip to next file
            if handled_as_workspace_cargo {
                continue;
            }
